...
import { Gestores, Projecto } from "./util";
...
import gestoresRoutes from "app/views/projecto/Gestores/routes";
...
export default function Listar() {
    ...
    console.log("CLIENTE ID", useParams());
    // let clienteId=1
    ...
    console.log();
    const filteredgestores = gestores?.filter((gestores) =>
        gestores?.nome.toLowerCase().includes(searchTerm?.toLowerCase())
    );
        ...
        <AppButtonRoot>
            <div className="w-100 d-flex  justify-content-between">
                ...
                <div>
                    ...
                    <Link to={`/clientes/${clienteId}/gestores/add`}>
                                    ...
                                    {filteredgestores
                                        ...
                                        ?.map((gest, index) => (
                                            <TableRow key={index} hover>
                                                ...
                                                <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                                                    <Paragraph style={{ fontSize: fSize }}>
                                                        {formatDateDifference(new Date(gest?.createdAt))}
                                                    </Paragraph>
                                                </TableCell>
                                                <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                                                    <CFormSelect
                                                        ...
                                                        onChange={async (e) => {
                                                            if (e.target.value == 1) {
                                                                return goto(
                                                                    `/clientes/${clienteId}/gestores/${gest?.id}/detalhar`
                                                                );
                                                            }
                                                            if (e.target.value == 2) {
                                                                return goto(
                                                                    `/gestores/editar/${gest?.id}`
                                                                );
                                                            }

                                                            if (e.target.value == 3) {
                                                                console.log("SOLICI ID", gest?.id);
                                                                setPedido(prev => gest?.id);

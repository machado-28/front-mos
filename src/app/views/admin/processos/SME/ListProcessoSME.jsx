...
export default function ListarProcessoSME() {
  ...
  const buscarRelario = async () => {
    try {
      await api.list("pedido-emissao/count?fazeId=4").then((resp) => {
        setTotalPedidoSubmetido(resp?.data?.total);
      });

      await api.listQuery(`pedido-emissao/count?statusId=6&fazeId=4`).then((resp) => {
        setTotalRecusados(resp?.data?.total);
      });

      await api.listQuery(`pedido-emissao/count?fazeId=4&statusId=5`).then((resp) => {
  ...
  async function ListarPedidos() {
    try {
      if (statusId === null && tipoVistoId === null) {
        ...
        await api
          .listQuery(`pedido-emissao/list?fazeId=4`)
          .then((resp) => {
      ...
      if (statusId !== null && tipoVistoId === null) {
        setLoadingPedido((prev) => !prev);
        await api
          .listQuery(`pedido-emissao/list?fazeId=4&statusId=${statusId}`)
          .then((resp) => {
            console.log("Pedido Recebidos", resp);
            const pedidos = resp?.data?.pedidos || [];
            console.log(pedidos);
      ...
      if (statusId === null && tipoVistoId !== null) {
        ...
        setLoadingPedido((prev) => !prev);
        await api
          .listQuery(`pedido-emissao/list?fazeId=4&tipoVistoId=${tipoVistoId}`)
          .then((resp) => {
            console.log("Pedido Recebidos", resp);
            const pedidos = resp?.data?.pedidos || [];
            console.log(pedidos);

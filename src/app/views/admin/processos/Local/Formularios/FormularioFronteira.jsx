...
export default function FormularioFronteira() {
  ...
  async function PostData(dados) {
    try {
      setLoading(true);
      const response = await api.add("pedido-emissao/4", dados).then(async (response) => {
        setLoading(false);
        const { data } = response;
        const { pedido } = data;
  ...
  return (
    <CForm onSubmit={handleSubmit(PostData)} style={{ borderRadius: "none" }}>
      <Box pt={2}></Box>
      <SimpleCard
        title={" FORMULARIO DE PEDIDO DE VISTO  DE FRONTEIRA"}

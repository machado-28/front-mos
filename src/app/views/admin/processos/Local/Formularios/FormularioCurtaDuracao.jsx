...
export default function FormularioCurtaduracao() {
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      if (fileSizeInKB > 2048) {
        ...
        (event.target.files[0].tipoId = Number(event.target.id.split("-")[1])),
  ...
  async function PostFile({
    ...
    console.log("DOCUMENTO", numeroPassaporte, dataValidade);
    ...
    try {
      ...
      if (file.tipoId === 1) {
        ...
        const response = await api
          .add(
            `upload?requerenteId=${requerenteId}&pedidoId=${pedidoId}&tipoId=${file.tipoId}` +
            file.tipoId,
            formData
          )
          .catch(({ error }) => {
  ...
  async function PostData(dados) {
    try {
      setLoading(true);
      const response = await api.add("pedido-emissao/3", dados).then(async (response) => {
        setLoading(false);
        const { data } = response;
        const { pedido } = data;
  ...
  return (
    <CForm onSubmit={handleSubmit(PostData)} style={{ borderRadius: "none" }}>
      <Box pt={2}></Box>
      <SimpleCard
        title={" FORMULARIO DE PEDIDO DE VISTO  DE CURTADURAÇÃO"}

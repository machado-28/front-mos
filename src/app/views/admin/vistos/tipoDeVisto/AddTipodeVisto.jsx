...
export default function AddTipodeVisto() {
  ...
  const api = useApi();
  const [tipodevistos, setStatusTipodeVisto] = useState([]);
  const redirectTo = useNavigate();

  async function buscarStatus() {
    await api.list("tipodevistos").then((response) => {
  ...
  async function PostData(data) {
    ...
    const response = await api
      .add('tipodevisto', data).then((response) => {
  ...
  return (
    <AppButtonRoot>
      ...
      <SimpleCard title="">
        <H3> REGISTO DE TIPO DE VISTO</H3>
      </SimpleCard>
      <Box py="12px" />

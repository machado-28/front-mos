...
export default function AddTipodeDocumento() {
  ...
  const api = useApi();
  const [tipodedocumento, setStatusTipodeDocumento] = useState([]);
  const redirectTo = useNavigate();
  const { id } = useParams();

  async function buscarTipoDocumento() {
    await api.list("documentos/tipo" + id).then((response) => {
  ...
  async function PostData(data) {
    ...
    const response = await api
      .edit('documentos/tipo', tipodedocumento).then((response) => {
        if (response.status === 201) {
          const { data } = response
          console.log("DATA", data);
          setStatusTipodeDocumento(data)
  ...
  return (
    <AppButtonRoot>
      ...
      <SimpleCard title="">
        <H3> ACTUALIZAR DE TIPO DE DOCUMENTO</H3>
      </SimpleCard>
      <Box py="12px" />

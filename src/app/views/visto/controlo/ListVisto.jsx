...
export default function Listar() {
  ...
  const [totalVistoLocal, setTotalVistoLocal] = useState(0);
  const [totalVistoSite, setTotalVistoSite] = useState(0);
  ...
  const buscarTotalVistoSite = async () => {
    setLoading((prev) => !prev);
    await api.listQuery(`pedidos?fromSite=${true}`).then((resp) => {
      console.log("pedidos", resp);
      setTotalVistoSite(prev => resp?.data?.total || 0)
    })
    setLoading((prev) => !prev);
  }

  const buscarTotalVistoLocal = async () => {
    setLoading((prev) => !prev);
    await api.listQuery(`pedidos?fromSite=${false || null}`).then((resp) => {
      console.log("pedidos", resp);
      setTotalVistoLocal(prev => resp?.data?.total || 0)
    })
    setLoading((prev) => !prev);
  }

  const buscarTotalVistoVisto = async ({ vistoId }) => {
    setLoading((prev) => !prev);
    ...
    try {
      ...
      await api
        ...
        .then((resp) => {
          ...
          if (resp.request?.status === 200) {
            setSolicitacoes(prev => resp?.data?.pedidos || []);
            setTotalVisto(prev => resp?.data?.total)
          }
  ...
  useEffect(() => {
    buscarTotalVistoLocal();

  }, [totalVistoLocal,])
  useEffect(() => {
    buscarTotalVistoSite();
  }, [totalVistoSite,]);
  ...
  return (
    <AppButtonRoot>
      ...
      <div className="w-100 d-flex  justify-content-between">
        <Title>LISTA DE VISTO ({totalVisto}) </Title>
        ...
        <div role="group" aria-label="Basic radio toggle button group flex-0">
          ...
          <StyledButton
            style={{ fontSize: "0.54rem", minWidth: "2.45rem", maxWidth: "5.45rem", borderRadius: 0 }}
            onClick={async () => { setOrigemId((prev) => 1); await ListarSolicitacoes() }}
            className="m-0"
            variant={OrigemId === 1 ? "contained" : "outlined"}
            color="primary"
          >
            Expirados({totalVistoLocal})
          </StyledButton>
          <StyledButton
            style={{ fontSize: "0.54rem", minWidth: "2.45rem", maxWidth: "5.45rem", borderRadius: 0 }}
            onClick={async () => { setOrigemId((prev) => 2); }}
            className="m-0"
            variant={OrigemId === 2 ? "contained" : "outlined"}
            color="primary"
          >
            Renovados({totalVistoSite})
          </StyledButton>
          <StyledButton
            style={{ fontSize: "0.54rem", minWidth: "2.45rem", maxWidth: "5.45rem", borderRadius: 0 }}
            onClick={async () => { setOrigemId((prev) => 3); }}
            className="m-0"
            variant={OrigemId === 3 ? "contained" : "outlined"}
            color="primary"
          >
            Cancelados({totalVistoSite})
          </StyledButton>
          ...
          <Link to={"/vistos/add"}>
            <StyledButton style={{ fontSize: "0.54rem", minWidth: "2.45rem", maxWidth: "9.45rem", borderRadius: 0 }} variant="outlined" color="success">
              Criar Novo
            </StyledButton>
          </Link>
          <div>
                    ...
                    ?.map((solicitacao, index) => (
                      <TableRow key={index} hover>
                        ...
                        <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                          ...
                          {/* {StatusBadge({ status: solicitacao?.status?.id })} */}
                          07/06/2024
                        </TableCell>
                        <TableCell sx={{ px: 0 }} align="left" colSpan={2}>

                          {/* <CBadge className={solicitacao?.fromSite ? "bg-info text-black" : "bg-success text-white"}> 
                               {{solicitacao?.fromSite ? "Site" : "Local"} }
                               </CBadge> */}
                          07/12/2024
                        </TableCell>

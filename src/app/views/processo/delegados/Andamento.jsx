...
export default function Listar() {
    const {
        ...
        resolver: zodResolver(Concluirshema),
    ...
    const [visibleRegistarVisto, setVisibleConcluir] = useState(false);
    ...
    const buscarProcesso = async (data) => {
        ...
        const res = await processo.buscarDelegados({ statusId: 2, responsavelId, clienteId, gestorId, projectoId })
    ...
    async function Concluir(data) {
        console.log(data);
        data.statusId = 4;
        setLoadingConcluir(prev => true)
        const processo = new Processo()
        await processo.concluir({ id: processoInfo?.id, data })
        setLoadingConcluir(prev => false)
        setVisibleConcluir(prev => false)
    }

    const [visibleRecusar, setVisibleRecusar] = useState(false)

    async function Recusar(data) {
        console.log(data);
        data.statusId = 5;
        setLoadingConcluir(prev => true)
        const processo = new Processo()
        await processo.concluir({ id: processoInfo?.id, data })
        setLoadingConcluir(prev => false)
        setVisibleRecusar(prev => false)
    }

    function FormVistoAdd() {
        return (
            <CForm onSubmit={handleSubmit(Concluir)}>
                <CModalBody>
                ...
                <CModalFooter>
                    <CButton color="secondary" onClick={() => setVisibleConcluir(false)}>
                        Cancelar
                    </CButton>
                    {loadingConcluir ? (
                        <CSpinner></CSpinner>
    ...
    function FormRecusar() {
        return (
            <CForm onSubmit={handleSubmit(Recusar)}>
                <CModalBody>
                    <CAlert color="secondary">
                    </CAlert>
                    <CRow>
    ...
    return (
        <AppButtonRoot>
            ...
            <CAlert color="info"> <strong>Nota:</strong><i>Processos  iniciados pelo tecnico</i></CAlert>
            <>
                <CModal
                    alignment="center"
                    visible={visibleMapa}
            ...
            <>
                ...
                <CModal
                    ...
                    onClose={() => setVisibleConcluir(false)}
                    aria-labelledby="VerticallyCenteredExample"
                ><CModalHeader>
                        <CModalTitle id="VerticallyCenteredExample">Concluir processo</CModalTitle>
                    </CModalHeader>
                    <Link className="p-4 text-info" to={`/processos/${processoInfo?.processoId}/detail`}><strong> PROCESSO Nº </strong>{processoInfo?.processo?.numero}</Link>
                    <FormVistoAdd></FormVistoAdd>
                </CModal>
                <CModal
                    alignment="center"
                    visible={visibleRecusar}
                    onClose={() => setVisibleRecusar(false)}
                    aria-labelledby="VerticallyCenteredExample"
                ><CModalHeader>
                        <CModalTitle id="VerticallyCenteredExample">Marcar como Recusado</CModalTitle>
                    </CModalHeader>
                    <Link className="p-4 text-info" to={`/processos/${processoInfo?.processoId}/detail`}><strong> PROCESSO Nº </strong>{processoInfo?.processo?.numero}</Link>
                    <FormVistoAdd></FormVistoAdd>
                </CModal>
                                            ...
                                            <TableRow key={index} hover>
                                                ...
                                                <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                                                    <CFormSelect
                                                        ...
                                                        onChange={async (e) => {
                                                            if (e.target.value == 1) {
                                                                setVisibleConcluir((prev) => true);
                                                                handleProcesso({ processo: process })
                                                            }
                                                            if (e.target.value == 2) {
                                                                setVisibleRecusar((prev) => true);
                                                                handleProcesso({ processo: process })
                                                            }

...
import { calcularDiasDecorrido, formatDateDifference } from "app/utils/validate";
...
export default function Listar() {
    ...
    const buscarProcesso = async (data) => {
        ...
        const res = await processo.buscarDelegados({ statusId: 4, responsavelId, clienteId, gestorId, projectoId })
    ...
    const registarVisto = async ({ data }) => {
        setLoadingVisto(prev => true)
        const vistoClass = new Visto();
        const res = await vistoClass.criar({ data })
        setLoadingVisto(prev => false)
        setVisibleRegistarVisto(prev => false)
    }
                                    ...
                                    {filteredProcessos
                                        ...
                                        ?.map((processo, index) => (
                                            <TableRow key={index} hover>
                                                ...
                                                <TableCell sx={{ px: 2 }} align="left" colSpan={3}>
                                                    <Paragraph >{processo?.processo?.projecto?.nome}</Paragraph>
                                                </TableCell>
                                                <TableCell sx={{ px: 3 }} align="left" colSpan={3}  >
                                                    <Paragraph  >
                                                        {/* <CBadge className={(addDays(new Date(processo?.createdAt, processo
                                                            ?.prazo
                                                        ))).getDay() ? "bg-success text-black" : "bg-warning text-black"}>{new Date(addDays(processo?.createdAt, processo
                                                            ?.prazo
                                                        )).toDateString()}</CBadge> */}
                                                        <CBadge className="bg-danger"> {calcularDiasDecorrido(processo?.processo?.createdAt, processo?.dataConclusao)}</CBadge>

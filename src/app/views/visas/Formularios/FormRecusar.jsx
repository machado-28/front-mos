...
const addRecusarShema = z.object({
  descricao: z.string().min(1, { message: "Este campo é obrigatorio" })
});
export default function FormRecusar({ handleClose, processoId }) {
  const {
    register,
    reset,
    watch,
    ...
    resolver: zodResolver(addRecusarShema),
  ...
  async function PostData(dados) {
    try {
      ...
      const response = await api
        .edit(`processo/${processoId}?statusId=6`, dados)
        .then(async (response) => {
          const { data } = response;
          const { pedido } = data;
  ...
  return (
    <CForm onSubmit={handleSubmit(PostData)} style={{ borderRadius: "none" }}>
      <DialogTitle id="form-dialog-title">Negação de processo</DialogTitle>
      <DialogContent>

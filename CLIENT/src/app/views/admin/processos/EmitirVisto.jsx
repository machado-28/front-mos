    ...
    <AppButtonRoot>
          ...
          <form onSubmit={handleSubmit(registarVisto)}>
            ...
            {errors?.numeroPassaporte && (
              <div className="text-light bg-danger">
                {errors?.numeroPassaporte.message}
              </div>
            )}
            <div className="mb-3">
            ...
            {fileSize > 0 && (
              <div>Tamanho do arquivo: {formatFileSize(fileSize)}</div>
            )}
            <div className="mb-3">
      ...
      <SimpleCard>
        <div className="w-100 d-flex  justify-content-between">
          <Title>Processos Concluídos (prontos a emissão de visto)</Title>
        ...
        <CContainer className="d-flex justify-content-between">
          ...
          >
            ...
            <CDropdown style={styleDropdown}>
              <CDropdownToggle style={styleDropdown}>
                Tipo De Visto
              </CDropdownToggle>
                ...
                ?.map((processo, index) => (
                  <TableRow key={index} hover>
                    ...
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      <Paragraph>
                        {processo?.requerente?.passaporte}LDAO987LD32345H
                      </Paragraph>
                    </TableCell>
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      ...
                      {processo?.status ? (
                        processo?.status?.id % 2 === 0 ? (
                          <Small bgcolor={bgSecondary}>
                            {processo?.status?.name}submetido
                          </Small>
                        ) : (
                          <Small bgcolor={bgPrimary}>
                            {" "}
                            {processo?.status?.name}Aprovado
                          </Small>
                        )
                      ) : (
                        <Small bgcolor={bgError}>
                          {" "}
                          {processo?.status?.name} Recusado{" "}
                        </Small>
                      )}
                    </TableCell>
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>

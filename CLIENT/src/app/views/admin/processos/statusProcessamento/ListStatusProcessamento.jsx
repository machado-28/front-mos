...
export default function ListStatusProcessamento() {
  ...
  const [status, setStatusProcesso] = useState([]);
  const [total, settotal] = useState(0);
  const api = useApi();
  async function ListarStatus() {
    await api.list("processos/status/").then((response) => {
  ...
  return (
    <AppButtonRoot>
      ...
      <Box py="12px" />
      <SimpleCard title="">
        <H3> LISTAGEM DE STATUS DE PROCESSAMENTO</H3>
      </SimpleCard>
      <Box py="12px" />

...
export default function ListProcessoMIREMPET() {
  ...
  async function ListarPedidos() {
    try {
      ...
      if (fazeId !== null && (statusId !== null && tipoVistoId === null)) {
        await api.listQuery(`pedido-emissao/list?fazeId=${fazeId}&statusId=${statusId}`).then((resp) => {
          console.log("Pedido Recebidos", resp);
          const pedidos = resp?.data?.pedidos || [];
          console.log(pedidos);
          setPedidos(pedidos);
      ...
      if (fazeId !== null && (statusId === null && tipoVistoId !== null)) {
        console.log("VEIO");
        await api.listQuery(`pedido-emissao/list?fazeId=${fazeId}&tipoVistoId=${tipoVistoId}`).then((resp) => {
          console.log("Pedido Recebidos", resp);
          const pedidos = resp?.data?.pedidos || [];
          console.log(pedidos);
          setPedidos(pedidos);
      ...
      if (fazeId === null && (statusId !== null && tipoVistoId !== null)) {
        await api.listQuery(`pedido-emissao/list?tipoVistoId=${tipoVistoId}&statusId=${statusId}`).then((resp) => {
          console.log("Pedido Recebidos", resp);
          const pedidos = resp?.data?.pedidos || [];
          console.log(pedidos);
          setPedidos(pedidos);
  ...
  async function buscarEntidade() {
    try {
      await api.listQuery(`fazes`).then((resp) => {
        if (resp.status !== 200) {
          ...
          <div className="btn-group" role="group" aria-label="Basic radio toggle button group">
            ...
            <CDropdown style={styleDropdown}>
              ...
              <CDropdownMenu style={styleDropdown}>
                <CDropdownItem onClick={() => { setStatusId(prev => null); setTituloStatus(prev => "Status") }} href="#">Todos</CDropdownItem>
                {statusPedidos?.map((status) => (
                  <CDropdownItem onClick={() => { setStatusId(status?.id); setTituloStatus(prev => status?.nome) }} href="#">{status?.nome}</CDropdownItem>
                ))}
              </CDropdownMenu>
            </CDropdown>
          </div>
                  ...
                  <TableRow key={index} hover>
                    ...
                    <TableCell colSpan={4} align="left" sx={{ px: 0, textTransform: "capitalize" }}>
                      <Box display="flex" alignItems="center" gap={2}>
                        ...
                        <Paragraph style={{ fontSize: fSize }} >{processo?.requerente?.nome}</Paragraph >
                      </Box>
                    </TableCell>
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      <Paragraph style={{ fontSize: fSize }} >{processo?.requerente?.Documentos?.[0]?.numero}</Paragraph >
                    </TableCell>
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      <Paragraph style={{ fontSize: fSize }} >{processo?.tipoVisto?.nome}</Paragraph >
                    </TableCell>
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      <Paragraph style={{ fontSize: fSize }} >{new Date(processo?.createdAt).toDateString()}</Paragraph >
                    </TableCell>

                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      {renderColorStatus({ id: processo?.statusActual?.id, nome: processo?.statusActual?.nome })}
                    </TableCell>
                    <TableCell sx={{ px: 0 }} align="left" colSpan={2}>
                      <Paragraph style={{ fontSize: fSize }} >{new Date(processo?.createdAt).toDateString()}</Paragraph >
                    </TableCell>
